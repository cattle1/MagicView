...
CGFloat const MAGProgressMaxOffset = 1000000.f;

static const CGFloat MAGDefaultPadding = 4.f;
static const CGFloat MAGDefaultLabelFontSize = 16.f;
static const CGFloat MAGDefaultDetailsLabelFontSize = 12.f;


@interface MAGProgressHUD ()
...
@interface MAGProgressHUDRoundedButton : UIButton
@end


@implementation MAGProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MAGProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    MAGProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (MAGProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MAGProgressHUD *hud = (MAGProgressHUD *)subview;
            if (!hud.hasFinished) {
                return hud;
            }
        }
...
- (void)commonInit {
    ...
    _animationType = MAGProgressHUDAnimationFade;
    _mode = MAGProgressHUDModeIndeterminate;
    _margin = 20.0f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MAGProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MAGProgressHUDAnimationZoom) {
        type = animatingIn ? MAGProgressHUDAnimationZoomIn : MAGProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MAGProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MAGProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MAGProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MAGProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    MAGProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MAGProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MAGBackgroundView *backgroundView = [[MAGBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MAGProgressHUDBackgroundStyleSolidColor;
    ...
    MAGBackgroundView *bezelView = [MAGBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:MAGDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:MAGDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [MAGProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:MAGDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[MAGRoundProgressView class]];

    MAGProgressHUDMode mode = self.mode;
    if (mode == MAGProgressHUDModeIndeterminate) {
    ...
    else if (mode == MAGProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[MAGBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MAGProgressHUDModeDeterminate || mode == MAGProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[MAGRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == MAGProgressHUDModeAnnularDeterminate) {
            [(MAGRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == MAGProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == MAGProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MAGProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MAGProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MAGRoundProgressView class]]) {
        MAGRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MAGRoundProgressView appearanceWhenContainedIn:[MAGProgressHUD class], nil];
#else
        appearance = [MAGRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MAGProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MAGRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MAGRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MAGBarProgressView class]]) {
        MAGBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MAGBarProgressView appearanceWhenContainedIn:[MAGProgressHUD class], nil];
#else
        appearance = [MAGBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MAGProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MAGBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MAGBarProgressView *)indicator).lineColor = color;
        }
    } else {
...
- (void)updateBezelMotionEffects {
    MAGBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MAGDefaultPadding : 0.f;
...
- (void)setMode:(MAGProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MAGProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation MAGRoundProgressView
...
@implementation MAGBarProgressView
...
@implementation MAGBackgroundView

#pragma mark - Lifecycle

- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        _style = MAGProgressHUDBackgroundStyleBlur;
        if (@available(iOS 13.0, *)) {
            #if TARGET_OS_TV
...
- (void)setStyle:(MAGProgressHUDBackgroundStyle)style {
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
    }
...
- (void)updateForBackgroundStyle {
    ...
    MAGProgressHUDBackgroundStyle style = self.style;
    if (style == MAGProgressHUDBackgroundStyleBlur) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MAGProgressHUDBackgroundStyleBlur) {
        self.backgroundColor = self.color;
    } else {
        self.backgroundColor = self.color;
    }
...
@implementation MAGProgressHUDRoundedButton
