...
#import "MAGSliderView.h"
@interface MAGSliderView() {
...
- (instancetype)initWithFrame:(CGRect)frame {
    return [self initWithFrame:frame setting:[MAGSliderSetting defaultSetting]];
}
...
- (void)initData {
    ...
    if(_setting.layoutDirection == MAGSliderLayoutDirectionHorizontal) {
...
- (void)setSetting:(MAGSliderSetting *)setting {
    _setting = setting;
    [self initData];
    [self configView];
...
- (void)longGestureAction:(UIPanGestureRecognizer *)gesture {
    if(gesture.state == UIGestureRecognizerStateBegan) {
        if([self.delegate respondsToSelector:@selector(sliderViewDidBeginDrag:)]) {
            [self.delegate sliderViewDidBeginDrag:self];
        }
    }
    else if(gesture.state == UIGestureRecognizerStateChanged)
    {
        ...
        if([self.delegate respondsToSelector:@selector(sliderView:didChangePercent:)]) {
            _currentPercent = percent;
            [self.delegate sliderView:self didChangePercent:percent];
        }
        
    }
    else if(gesture.state == UIGestureRecognizerStateEnded) {
         if([self.delegate respondsToSelector:@selector(sliderViewDidEndDrag:)]) {
           [self.delegate sliderViewDidEndDrag:self];
       }
    }
}
