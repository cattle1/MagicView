...
+ (void)refreshWindow {
    for (UIWindow *window in UIApplication.sharedApplication.windows) {
        ...
        if (!CGSizeEqualToSize(window.bounds.size, UIScreen.mainScreen.bounds.size)) continue;
        
        for (UIView *view in window.subviews) {
            if ([view isMemberOfClass:NSClassFromString(@"UITransitionView")]) continue;
            [view refresh];
        }
    }
}
...
+ (void)setUserInterfaceStyle:(LLUserInterfaceStyle)userInterfaceStyle {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (@available(iOS 13.0, *)) {
            for (UIWindow *window in UIApplication.sharedApplication.windows) {
                if (!window.isHidden && window != LLDarkWindow.sharedInstance) {
